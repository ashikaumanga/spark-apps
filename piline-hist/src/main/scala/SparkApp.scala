...
object SparkApp {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val incrementalDF = session.read
      ...
      .csv("/Users/ashika.umagiliya/Desktop/binlog.csv").toDF
    ...
    val currentHistActiveDF =  session.read
      ...
      .where("active=true")
      .drop(col("etl_start_time_str"))
      .drop(col("etl_end_time_str"))
      .drop(col("active")) //remove 'active' column since its recalculated
   ...
   //union active historical partition and incremental data
    val combinedDf = currentHistActiveDF.union(incrementalDF)
    combinedDf.show()


    //create active Column,use the latest record as active=true
    val window = Window.orderBy(desc("etl_start_time")).partitionBy("pk")
    val unionActiveColumnAddedDF = combinedDf.withColumn("rank", rank.over(window))
        .withColumn("active", expr("rank==1"))

    unionActiveColumnAddedDF.show()

    //create etl_start_date , etl_end_date Column
    //val window2 = Window.orderBy( asc("etl_time")).partitionBy("pk")
    val finalUnionDF = unionActiveColumnAddedDF
      //.withColumn("etl_end_time", when(expr("deleted=true"),current_timestamp).otherwise(lag(col("etl_start_time"),1))
        .withColumn("etl_end_time_tmp", lag(col("etl_start_time"),1)
         .over(window))
        .withColumn("elt_end_time", when(expr("deleted==true AND rank==1"), to_timestamp(current_timestamp,"yyyy-MM-dd'T'HH:mm")).otherwise(col("etl_end_time")))
